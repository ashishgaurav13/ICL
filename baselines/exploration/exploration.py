...
class CostShapingCallback(callbacks.BaseCallback):
    ...
    def _initialize_cost_net(self,
        ...
        self.cost_net_optim = Adam(self.cost_net.parameters(), lr=5e-4)
    ...
    def _on_rollout_end(self):
        ...
        self.logger.update({
            'CostShaping/mean_shaped_cost': np.mean(shaped_cost),
            'CostShaping/min_shaped_cost': np.min(shaped_cost),
            'CostShaping/max_shaped_cost': np.max(shaped_cost)
        })
